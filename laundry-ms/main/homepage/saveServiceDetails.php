...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sqlRequest = "SELECT request_id FROM tbl_service_request WHERE customer_id =? AND order_status NOT IN ('completed', 'cancelled')";
    ...
    foreach ($requestIds as $requestId) {
        ...
        $sqlTransaction = "INSERT INTO tbl_transaction (request_id, customer_id, service_option_id, service_option_name, laundry_cycle, customer_address, total_amount, delivery_fee, pickup_fee, rush_fee, amount_tendered, money_change)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $sqlPickUpDate = "UPDATE tbl_service_request SET request_date = ? WHERE request_id = ?";
        ...
        $sqlUpdate = "UPDATE tbl_service_request SET order_status = 'completed' WHERE request_id = ? AND customer_id = ?";
